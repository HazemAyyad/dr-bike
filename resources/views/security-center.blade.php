...
<main class="wrap">
    <header class="top">
        ...
        <div class="actions"><a class="chip" href="{{ route('security-center.employee-points.index') }}">تصفير نقاط الموظفين</a><a class="chip" href="{{ route('security-center.accounting.index') }}">سلامة المحاسبة</a><span class="ip" title="عنوانك الحالي">{{ $currentIp }}</span><form method="POST" action="{{ route('security-center.logout') }}">@csrf<button class="logout">تسجيل خروج</button></form></div>
    </header>
